...
| `ADMIN_PANEL.md` | `apps/admin-panel/src/routes`, shared components, server loaders/actions, RBAC helpers | Partially aligned | RBAC matrix, navigation visibility, CARLA controls, condition overrides, sensor configuration, and persisted operator notes were updated; full browser walkthrough remains part of live E2E. |
...
| `CORE_API.md` | `services/core-api/src/lib/api.ts`, auth, command handling, event handling, WebSocket hub | Partially aligned | Build/tests pass and RBAC/notes/sensor driver surfaces were tightened; full route behavior still needs live gateway E2E and recovery validation. |
| `DATABASE_ENTITIES.md` | `infra/database/schema.sql`, seed data, CoreAPI data helpers | Aligned | Static schema tests pass for foundation tables, columns, summaries, devices, exports, and activity log coverage. |
...
| `OVERLAY_ENGINE.md` | `apps/overlay-web`, `apps/desktop-overlay`, overlay configure endpoints | External validation required | Overlay contracts/static tests pass; browser and Electron visual smoke checks remain required against a running stack. |
...
| `PROCESS_MANAGER.md` | `scarline`, `scarline.ps1`, `infra/process-manager/ipc_server.py` | Runtime blocked | Static launcher/process-manager tests pass and partial startup/stop cleanup worked; full `--no-carla` startup readiness is blocked by Docker image metadata resolution. |
...
| `WIDGETS.md` | Static widget architecture, `widget.json`, shared assets, `SCARline` JS API | Aligned | Widget metadata categories are canonical, each widget remains static HTML/JSON, and tests reject direct `fetch` or custom WebSocket usage. |
