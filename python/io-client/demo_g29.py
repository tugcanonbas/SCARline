...
async def run_dry() -> None:
    """Stub mode -- prints 3 synthetic events without hardware or RabbitMQ."""
    print("[DEMO G29] Stub mode -- no hardware, no RabbitMQ")
    for i, stub in enumerate(STUB_READINGS, 1):
        payload = build_payload(
            steering_angle=stub["steeringAngle"],
            throttle=stub["throttle"],
            brake=stub["brake"],
        )
        print(f"[DEMO G29] Event {i}: {json.dumps(payload)}")
    print("[DEMO G29] Done -- 3 events published")
...
async def run_live() -> None:
    ...
    while published < EVENT_COUNT:
        ...
        if reading is not None:
            ...
            print(f"[DEMO G29] Event {published}: {json.dumps(payload)}")
