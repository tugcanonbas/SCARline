...
<div class="layout-canvas-col">
  <div class="layout-canvas-toolbar">
    ...
    <label class="layout-canvas-display">
      ...
      <input
        ...
        type="text"
        oninput={(event) => {
          targetDisplay = (event.currentTarget as HTMLInputElement).value.replaceAll(/\D/g, '') || '0';
        }}
      />
    </label>
    <label class="layout-canvas-display">
  ...
  <p class="layout-canvas-hint">
    1920×1080 canvas scaled to {CANVAS_W}×{CANVAS_H}px · Drag to reposition · Click to select
  </p>
</div>
