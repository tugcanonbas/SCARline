  ...
  <div class="telemetry-chart-grid">
    <div class="telemetry-tile">
      <div class="telemetry-tile__header">
        ...
        <span class="telemetry-tile__value">{vehicleState.speed ?? '—'} <small>km/h</small></span>
      </div>
      {#if speedHistory.length > 1}
        <div class="telemetry-tile__chart">{@html miniChart(speedHistory, '#245f88', undefined)}</div>
      {:else}
    ...
    <div class="telemetry-tile">
      <div class="telemetry-tile__header">
        ...
        <span class="telemetry-tile__value">{Math.round(Number(vehicleState.throttle ?? 0) * 100)}%</span>
      </div>
      {#if throttleHistory.length > 1}
        <div class="telemetry-tile__chart">{@html miniChart(throttleHistory, '#1f7a45', 1)}</div>
      {:else}
        <div class="telemetry-tile__chart telemetry-tile__chart--empty">Awaiting stream</div>
      {/if}
    </div>
    <div class="telemetry-tile">
      <div class="telemetry-tile__header">
        ...
        <span class="telemetry-tile__value">{Math.round(Number(vehicleState.brake ?? 0) * 100)}%</span>
      </div>
      {#if brakeHistory.length > 1}
        <div class="telemetry-tile__chart">{@html miniChart(brakeHistory, '#946200', 1)}</div>
      {:else}
        <div class="telemetry-tile__chart telemetry-tile__chart--empty">Awaiting stream</div>
      {/if}
    </div>
    <div class="telemetry-tile">
      ...
      <div class="telemetry-tile__chart telemetry-tile__chart--empty">Heading: {vehicleState.heading ?? '—'}</div>
    </div>
  </div>
  <div class="mt-3 border-t border-[--color-line] pt-3">
    <KeyValueGrid
      items={[
        ...
        { label: 'Routing Key', value: String(vehicleState.routingKey ?? 'Waiting for stream') }
      ]}
    />
  </div>
</SurfaceCard>
