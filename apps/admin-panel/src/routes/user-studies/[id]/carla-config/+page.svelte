...
<div class="metric-grid">
  <MetricCard label="Map" value={data.config?.map ?? 'Unset'} hint="World loaded before session start" accent />
  <MetricCard label="Weather" value={data.config?.weatherPreset ?? data.config?.weather_preset ?? 'Default'} hint="Baseline before condition overrides" />
  <MetricCard label="Sensors" value={configuredSensors.length} hint="Simulator-attached devices" />
  <MetricCard label="NPC Vehicles" value={Number(trafficConfig.npcVehicleCount ?? trafficConfig.npc_vehicle_count ?? 15)} hint="Traffic density baseline" />
</div>
    ...
    <SurfaceCard title="Simulator Defaults" subtitle="These values are persisted as the study-level CARLA baseline.">
      <form class="form-stack" method="POST">
        ...
        <label class="form-field">
          ...
          <input min="0" name="npcVehicleCount" type="number" value={trafficConfig.npcVehicleCount ?? trafficConfig.npc_vehicle_count ?? 15} />
        </label>
        <div class="form-grid-3">
          <label class="form-field">
            <span>Pedestrians</span>
            <input min="0" name="pedestrianCount" type="number" value={pedestrianConfig.pedestrianCount ?? pedestrianConfig.pedestrian_count ?? 0} />
          </label>
          <label class="form-field">
            <span>Traffic Speed Difference</span>
            <input name="trafficSpeedDifference" type="number" value={trafficConfig.speedDifference ?? trafficConfig.speed_difference ?? 0} />
          </label>
          <label class="form-field">
            <span>Sun Altitude Angle</span>
            <input name="sunAltitudeAngle" type="number" value={sunConfig.sunAltitudeAngle ?? sunConfig.sun_altitude_angle ?? 45} />
          </label>
        </div>
        <div class="form-grid-3">
          <label class="form-field">
            <span>Cloudiness</span>
            <input max="100" min="0" name="cloudiness" type="number" value={weatherCustom.cloudiness ?? 0} />
          </label>
          <label class="form-field">
            <span>Precipitation</span>
            <input max="100" min="0" name="precipitation" type="number" value={weatherCustom.precipitation ?? 0} />
          </label>
          <label class="form-field">
            <span>Wind Intensity</span>
            <input max="100" min="0" name="windIntensity" type="number" value={weatherCustom.windIntensity ?? weatherCustom.wind_intensity ?? 0} />
          </label>
        </div>
        <div class="form-grid-2">
          ...
          <label class="form-field">
            ...
            <input name="fixedDeltaSeconds" step="0.01" type="number" value={data.config?.fixedDeltaSeconds ?? data.config?.fixed_delta_seconds ?? 0.05} />
          </label>
        </div>
        <div class="detail-panel">
          ...
          <div class="form-grid-3 mt-3">
            <label class="form-field form-field--compact"><span>Camera Width</span><input name="cameraWidth" type="number" value={sensorAttribute('sensor.camera.rgb', 'image_size_x', 1280)} /></label>
            <label class="form-field form-field--compact"><span>Camera Height</span><input name="cameraHeight" type="number" value={sensorAttribute('sensor.camera.rgb', 'image_size_y', 720)} /></label>
            <label class="form-field form-field--compact"><span>Camera FOV</span><input name="cameraFov" type="number" value={sensorAttribute('sensor.camera.rgb', 'fov', 90)} /></label>
            <label class="form-field form-field--compact"><span>LiDAR Range</span><input name="lidarRange" type="number" value={sensorAttribute('sensor.lidar.ray_cast', 'range', 80)} /></label>
            <label class="form-field form-field--compact"><span>LiDAR Channels</span><input name="lidarChannels" type="number" value={sensorAttribute('sensor.lidar.ray_cast', 'channels', 32)} /></label>
          </div>
        </div>
        <div class="form-grid-2">
          ...
          <label class="form-field md:col-span-2">
            ...
            <input name="recordingDirectory" value={recordingConfig.directory ?? ''} />
          </label>
        </div>
    ...
    <div class="mt-4 space-y-3">
      ...
      {#each configuredSensors as sensor}
        <div class="entity-card entity-card--tight">
          ...
          <p class="entity-card__meta">{sensor.type} · transform x={sensor.transform?.x ?? 0}, y={sensor.transform?.y ?? 0}, z={sensor.transform?.z ?? 0}</p>
        </div>
      {:else}
