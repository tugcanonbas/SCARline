...
function normalizeBounds(bounds = {}) {
  return {
    ...
    width: Number.isFinite(bounds.width) ? Math.max(1, Number(bounds.width)) : 180,
    height: Number.isFinite(bounds.height) ? Math.max(1, Number(bounds.height)) : 180
  };
}
...
function createWindowOptions(spec) {
  const bounds = normalizeBounds(spec.bounds);
  ...
  return {
    ...
    focusable: transparent ? process.env.OVERLAY_FOCUSABLE === 'true' : true,
...
function applyWindowBehavior(windowRef, spec) {
  ...
  const transparent = mode === 'transparent_electron';
  if (transparent) {
    const clickThrough = spec.clickThrough !== false && globalClickThrough;
...
function createManagedWindow(spec) {
  ...
  windowsById.set(normalizedSpec.instanceId, { window: windowRef, spec: normalizedSpec });
  return windowRef;
}
...
function updateManagedWindow(spec) {
  ...
  const nextSpec = {
    ...
    clickThrough: typeof spec.clickThrough === 'boolean' ? spec.clickThrough : existing.spec.clickThrough,
    url: typeof spec.url === 'string' && spec.url.length > 0 ? spec.url : existing.spec.url,
    bounds: normalizeBounds(spec.bounds ?? existing.spec.bounds)
  };

  if (nextSpec.mode !== existing.spec.mode) {
    destroyManagedWindow(id);
    createManagedWindow({ ...nextSpec, instanceId: id, widgetId: existing.spec.widgetId });
    return;
  }

  existing.window.setBounds(nextSpec.bounds);
  applyWindowBehavior(existing.window, nextSpec);
  if (nextSpec.url !== existing.spec.url) {
    existing.window.loadURL(nextSpec.url);
  }
  windowsById.set(id, { window: existing.window, spec: { ...nextSpec, instanceId: id, widgetId: existing.spec.widgetId } });
}
...
function syncManagedWindows() {
  const nextSpecs = runtimeConfig.windows
    .filter((entry) => entry && typeof entry.instanceId === 'string' && entry.instanceId && entry.url)
    .map((entry) => ({
...
function configureOverlay(payload = {}) {
  ...
  if (Array.isArray(payload.windows)) {
    runtimeConfig.windows = payload.windows.map((entry) => ({
      instanceId: String(entry.instanceId ?? ''),
      widgetId: String(entry.widgetId ?? ''),
      mode: normalizeMode(entry.mode),
      url: String(entry.url ?? ''),
...
function updateWindows(payload = {}) {
  ...
  const byId = new Map(runtimeConfig.windows.map((entry) => [entry.instanceId, entry]));
  for (const update of payload.windows) {
    ...
    const next = {
      ...
      bounds: normalizeBounds({
        ...
        height: update.bounds?.height ?? update.height ?? existing.bounds.height
      })
    };
...
function openWindows(payload = {}) {
  ...
  if (payload.session && typeof payload.session === 'object') {
    runtimeConfig.session = {
      ...
      conditionId: payload.session.conditionId ?? runtimeConfig.session.conditionId
    };
  }
...
function startControlServer() {
  ...
  controlServer.on('error', (error) => {
    console.error(`Overlay control server failed: ${error instanceof Error ? error.message : String(error)}`);
  });
