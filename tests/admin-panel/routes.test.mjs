...
test("admin routes do not mix default and named page actions", async () => {
  for (const routeFile of [
    ...
    const namedActions = Array.from(
      source.matchAll(/^\s*([a-zA-Z][\w]*):\s*async/gm),
    )
...
test("admin panel route RBAC matches documented role matrix", async () => {
  ...
  const activeStudy = await readRoute(
    "user-studies/[id]/active-study/+page.server.ts",
  );
  assert.match(
    activeStudy,
    /requireRole\([^)]*\['admin', 'researcher', 'operator'\]\)/,
  );
  assert.doesNotMatch(
    activeStudy,
    /\['admin', 'researcher', 'operator', 'viewer'\]/,
...
test("admin navigation and nginx route PRD admin paths through the Svelte shell", async () => {
  ...
  const navItem = await readFile(
    path.join(
      root,
      "apps/admin-panel/src/lib/components/admin/shell/AppShellNavItem.svelte",
    ),
    "utf8",
  );
  const nginx = await readFile(
    path.join(root, "infra/nginx/default.conf"),
    "utf8",
  );
  ...
  assert.match(layout, /external:\s*true/);
  assert.match(layout, /docsUrl/);
  assert.match(layout, /href:\s*data\.docsUrl \?\? "\/docs\/"/);
  assert.match(navItem, /target=\{item\.external \? '_blank' : undefined\}/);
  assert.match(navItem, /rel=\{item\.external \? 'noopener noreferrer' : undefined\}/);
  assert.doesNotMatch(
    nginx,
    /location = \/documentation\s*\{\s*return 308 \/docs\//,
  );
...
test("admin auth handles real logout and bootstrap outages explicitly", async () => {
  ...
  const hooks = await readFile(
    path.join(root, "apps/admin-panel/src/hooks.server.ts"),
    "utf8",
  );
  const auth = await readFile(
    path.join(root, "apps/admin-panel/src/lib/server/auth.ts"),
    "utf8",
  );
  const bootstrap = await readFile(
    path.join(root, "apps/admin-panel/src/lib/server/bootstrap.ts"),
    "utf8",
  );
  ...
  assert.match(layout, /throw error\(\s*503,/);
  assert.match(
    layout,
    /target === "\/login" \? createLoginRedirectPath\(url\)/,
  );
});

test("admin realtime store reconnects and resubscribes after socket drops", async () => {
  const source = await readFile(
    path.join(root, "apps/admin-panel/src/lib/stores/realtime.ts"),
    "utf8",
  );
...
test("dashboard and active study realtime updates avoid self-tracking effect loops", async () => {
  ...
  assert.doesNotMatch(
    dashboard,
    /recentSessions = \[\s*normalized,\s*\.\.\.recentSessions/,
  );
  assert.doesNotMatch(
    dashboard,
    /componentHealth = \[\s*normalized,\s*\.\.\.componentHealth/,
  );
  ...
  assert.doesNotMatch(
    activeStudy,
    /speedHistory = \[\.\.\.speedHistory\.slice/,
  );
  assert.doesNotMatch(
    activeStudy,
    /throttleHistory = \[\.\.\.throttleHistory\.slice/,
  );
  assert.doesNotMatch(
    activeStudy,
    /brakeHistory = \[\.\.\.brakeHistory\.slice/,
  );
});

test("admin shell publishes icon assets for root browser probes", async () => {
  const appHtml = await readFile(
    path.join(root, "apps/admin-panel/src/app.html"),
    "utf8",
  );
