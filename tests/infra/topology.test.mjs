...
test("rabbitmq definitions expose milestone queues and exchanges", async () => {
  const definitions = JSON.parse(
    await readFile(path.join(root, "infra/rabbitmq/definitions.json"), "utf8"),
  );
  const exchanges = new Set(
    definitions.exchanges.map((exchange) => exchange.name),
  );
...
test("scarline launcher advertises required commands and IPC socket", async () => {
  ...
  assert.match(
    script,
    /core-api sim-bridge admin-panel overlay-web docs io-client nginx/,
  );
  assert.match(script, /open_admin_ui/);
  assert.match(
    script,
    /wait_for_health core-api sim-bridge admin-panel overlay-web docs io-client nginx/,
  );
...
test("database schema includes PRD foundation tables and columns", async () => {
  const schema = await readFile(
    path.join(root, "infra/database/schema.sql"),
    "utf8",
  );
  ...
  assert.match(
    schema,
    /ALTER TABLE devices ADD COLUMN IF NOT EXISTS display_configuration/,
  );
  assert.match(
    schema,
    /ALTER TABLE export_jobs ADD COLUMN IF NOT EXISTS parameters/,
  );
...
test("nginx exposes PRD-aligned single-domain routing", async () => {
  ...
  assert.match(
    config,
    /proxy_pass http:\/\/admin-panel:3000\/admin\/favicon\.ico;/,
  );
  assert.match(
    config,
    /proxy_pass http:\/\/admin-panel:3000\/admin\/apple-touch-icon\.png;/,
  );
  assert.match(
    config,
    /proxy_pass http:\/\/admin-panel:3000\/admin\/apple-touch-icon-precomposed\.png;/,
  );
...
test("process manager ipc can relay overlay reloads to electron control endpoint", async () => {
  const source = await readFile(
    path.join(root, "infra/process-manager/ipc_server.py"),
    "utf8",
  );
...
test("desktop overlay exposes health and reload control hooks", async () => {
  const source = await readFile(
    path.join(root, "apps/desktop-overlay/src/main.mjs"),
    "utf8",
  );
  assert.match(source, /OVERLAY_CONTROL_PORT/);
  ...
  assert.match(
    source,
    /backgroundMaterial:\s*transparent \? 'none' : undefined/,
  );
  ...
  assert.match(
    source,
    /setFocusable\(process\.env\.OVERLAY_FOCUSABLE === 'true'\)/,
  );
...
test("overlay web exposes widget validation and relative gateway assets", async () => {
  const source = await readFile(
    path.join(root, "apps/overlay-web/src/server.ts"),
    "utf8",
  );
...
test("launcher and compose configure internal status auth, dev seed, and healthy gateway dependencies", async () => {
  ...
  assert.match(compose, /dockerfile: docs\/Dockerfile/);
});
